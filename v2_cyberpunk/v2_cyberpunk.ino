...
void drawRadar() {
  ...
  // Calculate Farthest Device Distance & Render Dynamic Header replacing static RADAR text
  float farthestDist = (totalCount > 0) ? items[totalCount - 1].dist : 0.0f;
  char headerBuf[20];
  if (totalCount > 0) {
    snprintf(headerBuf, sizeof(headerBuf), "%dD %.0fm", totalCount, farthestDist);
  } else {
    snprintf(headerBuf, sizeof(headerBuf), "0D 0m");
  }
  ...
  for (int i = 0; i < totalCount; i++) {
    ...
    if (targetLockActive && memcmp(items[i].mac, target_mac, 6) == 0) {
      ...
      canvas.drawCircle(bx, by, 6, FIX_RED);
      canvas.drawCircle(bx, by, 8, FIX_YELLOW);
    }
  }
  ...
  for (int i = 0; i < totalCount && i < 4; i++) {
    // Line 1: Type indicator + Distance
    if (items[i].isAP) {
