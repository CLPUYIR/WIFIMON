...
"<title>WIFIMON V3 - Hybrid Wi-Fi & BLE Radar</title>"
"<style>"
...
"h1 { font-size: 1.1rem; color: #06b6d4; margin: 0; font-weight: 800; letter-spacing: 1px; }"
...
"button.magenta { background: #ec4899; color: #fff; }"
...
".tag-bt { color: #ec4899; font-weight: bold; }"
"</style></head><body>"
"<div class='header'><h1>⚡ WIFIMON V3 HYBRID</h1><div id='modeBadge' class='badge'>WF RADAR</div></div>"
"<div class='radar-box'><canvas id='radar' width='340' height='340'></canvas></div>"
"<div class='controls'>"
"<div class='row'>"
"<button onclick='setMode(\"WIFI\")'>Wi-Fi (10s)</button>"
"<button class='magenta' onclick='setMode(\"BLE\")'>BLE (10s)</button>"
"<button onclick='setMode(\"AUTO\")'>Auto-Cycle</button>"
"</div>"
...
"<div class='row'><button class='red' style='flex:1;' onclick='resetDb()'>Reset All Radar Databases</button></div>"
"</div>"
"<div class='card'>"
"<div class='card-title'><span id='tableTitle'>SENSED DEVICES</span><span id='devCount'>0 Devices</span></div>"
"<table><thead><tr><th>Type</th><th>Name / Vendor</th><th>Dist</th><th>Action</th></tr></thead>"
"<tbody id='deviceTable'></tbody></table>"
"</div>"
"<script>"
"let blips = []; let targetMac = ''; let targetLockActive = false; let activeTab = 'AUTO';"
...
"    ctx.fillStyle = b.isBT ? '#ec4899' : (b.isAP ? '#eab308' : (b.dist < 5 ? '#22c55e' : '#06b6d4'));"
...
"    const isBLEMode = (activeTab === 'BLE' || (activeTab === 'AUTO' && d.radarMode === 'BLE'));"
"    document.getElementById('modeBadge').innerText = isBLEMode ? 'BT BLE RADAR' : 'WF RADAR';"
"    document.getElementById('modeBadge').style.background = isBLEMode ? '#ec4899' : '#06b6d4';"
"    targetLockActive = d.targetLockActive; targetMac = d.targetMac;"
"    blips = []; let html = '';"
"    const devList = isBLEMode ? d.bleDevices : d.wifiDevices;"
"    document.getElementById('devCount').innerText = devList.length + (isBLEMode ? ' BLE Devices' : ' Wi-Fi Devices');"
"    devList.forEach(item => {"
"      const angle = (item.hash % 360) * Math.PI / 180;"
"      blips.push({ mac: item.mac, dist: item.dist, angle: angle, isAP: item.isAP, isBT: isBLEMode });"
"      html += `<tr>` +"
"        `<td class='${isBLEMode ? 'tag-bt' : (item.isAP ? 'tag-ap' : 'tag-cl')}'>${isBLEMode ? 'BT' : (item.isAP ? 'AP' : 'CL')}</td>` +"
...
"function setMode(m) { activeTab = m; updateData(); }"
...
// --- Cyberpunk Feature State Variables ---
bool targetLockActive = false;
uint8_t target_mac[6] = {0};
bool crtFilterEnabled = false; // Disabled by default for maximum brightness
String marqueeText = "";
int marqueeX = 160;

// Short OUI Vendor tags for Micro Badges
const char* getMicroVendorTag(uint8_t* mac) {
    const char* full = getVendor(mac);
    if (strcmp(full, "Apple") == 0) return "APL";
    if (strcmp(full, "Samsung") == 0) return "SAM";
    if (strcmp(full, "Espressif") == 0) return "ESP";
    if (strcmp(full, "Intel") == 0) return "INT";
    if (strcmp(full, "Google") == 0) return "GGL";
    if (strcmp(full, "Oppo/OnePlus") == 0) return "OPP";
    if (strcmp(full, "Xiaomi") == 0) return "XIA";
    if (strcmp(full, "Huawei") == 0) return "HWA";
    if (strcmp(full, "TP-Link") == 0) return "TPL";
    if (strcmp(full, "Netgear") == 0) return "NET";
    if (strcmp(full, "Randomized") == 0) return "RND";
    return "UNK";
}

// CRT Scanline Filter Post-Processor
void applyScanlines() {
    ...
    for (int y = 0; y < 128; y += 3) {
        ...
        for (int x = 0; x < 160; x++) {
            ...
            g = (g * 3) / 4;
            b = (b * 3) / 4;
            buf[rowStart + x] = (r << 11) | (g << 5) | b;
        }
    }
}

// Fast, Smooth & High-Contrast 2.4GHz Channel Hopping Tape (Header top-right X: 100 to 159, Y: 1 to 14)
void drawChannelTape() {
    ...
    // Draw tape frame & background
    canvas.fillRect(tapeX, tapeY, tapeW, tapeH, FIX_BLACK);
    canvas.drawRect(tapeX, tapeY, tapeW, tapeH, FIX_CYAN);

    // Smooth scroll position interpolation based on currentChannel (Lerp)
    float targetPos = (float)(currentChannel - 1) * 10.0f;
    static float scrollPos = 0.0f;
    scrollPos += (targetPos - scrollPos) * 0.45f; // Fast fluid slide transition

    int centerX = tapeX + (tapeW / 2);

    // Render sliding channel ticks & numbers 1 to 13
    for (int ch = 1; ch <= 13; ch++) {
        float chX = centerX + (float)(ch - 1) * 10.0f - scrollPos;
        int intX = (int)chX;

        // Only draw if inside tape window bounds
        if (intX >= tapeX + 2 && intX <= tapeX + tapeW - 6) {
            if (ch == currentChannel) {
                // Active Channel: Highlighted Cyan Badge
                canvas.fillRect(intX - 4, tapeY + 1, 9, tapeH - 2, FIX_CYAN);
                canvas.setCursor(intX - 3, tapeY + 3);
                canvas.setTextColor(FIX_BLACK);
                canvas.printf("%d", ch);
            } else {
                // Inactive Channels: High contrast White text & ticks
...
// Airspace Threat Score & Grade Calculation
int calculateThreatScore(char &gradeChar) {
    int openAPCount = 0;
    int totalAPCount = 0;
    int randClientCount = 0;
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (trackedClients[i].active) {
            totalClientCount++;
            if (trackedClients[i].mac[0] & 0x02) {
                randClientCount++;
            }
        }
    }
    ...
    int score = 0;
    if (totalAPCount > 0) {
        score += (openAPCount * 50) / totalAPCount;
    }
    if (totalClientCount > 0) {
        score += (randClientCount * 30) / totalClientCount;
    }
    if (score > 100) score = 100;

    if (score < 20) gradeChar = 'A';
    else if (score < 40) gradeChar = 'B';
    else if (score < 60) gradeChar = 'C';
    else if (score < 80) gradeChar = 'D';
    else gradeChar = 'F';

    return score;
}

// Fast & High-Contrast Probed SSID Marquee Ticker
void drawMarqueeTicker() {
    if (marqueeText.length() == 0) return;
    canvas.fillRect(0, 116, 160, 12, FIX_BLACK);
    canvas.drawFastHLine(0, 115, 160, FIX_CYAN);
    canvas.setCursor(marqueeX, 118);
    canvas.setTextColor(FIX_YELLOW);
    canvas.setTextWrap(false);
    canvas.print(marqueeText);

    marqueeX -= 8; // Fast smooth scrolling step
    int textPixelWidth = marqueeText.length() * 6;
...
void update_client(uint8_t* mac, int rssi, const char* ssid, unsigned long now) {
  ...
  if (ssid && strlen(ssid) > 0) {
    ...
    char tickerMsg[48];
    snprintf(tickerMsg, sizeof(tickerMsg), "[!] %02X:%02X -> %s | ", mac[4], mac[5], ssid);
    if (marqueeText.length() > 200) marqueeText = "";
    marqueeText += tickerMsg;
  } else if (foundIdx < 0) {
...
void update_ble(uint8_t* mac, int rssi, const char* name, const char* vendor, unsigned long now) {
  int foundIdx = -1;
  int emptyIdx = -1;
  ...
  if (name && strlen(name) > 0) {
    strncpy(trackedBLE[insertIdx].name, name, 32);
    trackedBLE[insertIdx].name[32] = '\0';
  } else {
    strncpy(trackedBLE[insertIdx].name, vendor, 32);
    trackedBLE[insertIdx].name[32] = '\0';
  }
  strncpy(trackedBLE[insertIdx].vendor, vendor, 15);
  trackedBLE[insertIdx].vendor[15] = '\0';
    ...
    void onResult(BLEAdvertisedDevice advertisedDevice) {
      ...
      if (advertisedDevice.haveManufacturerData()) {
        ...
        if (mData.length() >= 2) {
          ...
          else if (companyId == 0x0041) vendor = "Tile Tracker";
          else if (companyId == 0x02E5) vendor = "Espressif";
        }
      }

      update_ble(mac, rssi, name.c_str(), vendor, now);
    }
};
...
void handleApiData() {
  ...
  json += "\"radarMode\":\"" + String((currentRadarMode == RADAR_WIFI) ? "WIFI" : "BLE") + "\",";
  char targetStr[18];
  ...
  for (int i = 0; i < MAX_APS; i++) {
    if (trackedAPs[i].active) {
      ...
      wifiJson += "{\"isAP\":true,\"mac\":\"" + String(bssidStr) + "\",\"name\":\"" + String(trackedAPs[i].ssid) + "\",\"dist\":" + String(dist, 1) + ",\"hash\":" + String(hash) + "}";
      activeWifiCount++;
    }
  }
  ...
  for (int i = 0; i < MAX_CLIENTS; i++) {
    if (trackedClients[i].active) {
      ...
      wifiJson += "{\"isAP\":false,\"mac\":\"" + String(macStr) + "\",\"name\":\"" + name + "\",\"dist\":" + String(dist, 1) + ",\"hash\":" + String(hash) + "}";
      activeWifiCount++;
    }
  }
  wifiJson += "]";

  int activeBleCount = 0;
  String bleJson = "[";
  for (int i = 0; i < MAX_BLE_DEVICES; i++) {
    if (trackedBLE[i].active) {
      if (activeBleCount > 0) bleJson += ",";
      float dist = pow(10.0f, (-40.0f - (float)trackedBLE[i].rssi) / 27.0f);
      uint32_t hash = (trackedBLE[i].mac[3] ^ trackedBLE[i].mac[4] ^ trackedBLE[i].mac[5]) * 360 / 256;
      char bleMacStr[18];
      snprintf(bleMacStr, sizeof(bleMacStr), "%02X:%02X:%02X:%02X:%02X:%02X",
               trackedBLE[i].mac[0], trackedBLE[i].mac[1], trackedBLE[i].mac[2],
               trackedBLE[i].mac[3], trackedBLE[i].mac[4], trackedBLE[i].mac[5]);
      
      bleJson += "{\"isAP\":false,\"mac\":\"" + String(bleMacStr) + "\",\"name\":\"" + String(trackedBLE[i].name) + "\",\"dist\":" + String(dist, 1) + ",\"hash\":" + String(hash) + "}";
      activeBleCount++;
    }
  }
  bleJson += "]";

  json += "\"wifiDevices\":" + wifiJson + ",";
  json += "\"bleDevices\":" + bleJson + "}";
...
void setup() {
  ...
  // Initialize BLE Device Scanner for Hybrid Sniffing
  ...
  Serial.println("[SYSTEM] Bluetooth BLE Scanner Initialized!");
...
void handleButton() {
  ...
  if (buttonState == LOW) { // Button is pressed (Active LOW)
    ...
    if (buttonWasPressed) {
      ...
      if (pressDuration > 50) {
        // --- BUTTON PRESS: Clear all tracked devices and target locks ---
        for (int i = 0; i < MAX_CLIENTS; i++) {
...
void processSerialCommand(String cmd) {
  ...
  if (cmd.startsWith("TIME:")) {
    ...
    if (sscanf(macStr.c_str(), "%x:%x:%x:%x:%x:%x", &parsed[0], &parsed[1], &parsed[2], &parsed[3], &parsed[4], &parsed[5]) == 6) {
      for (int k = 0; k < 6; k++) target_mac[k] = (uint8_t)parsed[k];
      targetLockActive = true;
      Serial.printf("\r\n\033[1;31m[TARGET LOCK] Target set: %02X:%02X:%02X:%02X:%02X:%02X\033[0m\r\n",
                    target_mac[0], target_mac[1], target_mac[2], target_mac[3], target_mac[4], target_mac[5]);
    }
  } else if (cmd.equalsIgnoreCase("unlock")) {
    targetLockActive = false;
    Serial.println("\r\n\033[32m[TARGET LOCK] Target vector unlocked.\033[0m");
  } else if (cmd.equalsIgnoreCase("crt")) {
    crtFilterEnabled = !crtFilterEnabled;
    Serial.printf("\r\n\033[35m[UI] CRT scanline filter: %s\033[0m\r\n", crtFilterEnabled ? "ENABLED" : "DISABLED");
  } else if (cmd.equalsIgnoreCase("h") || cmd.equalsIgnoreCase("?")) {
    Serial.println("\r\n\033[35m--- ESP32 WIFIMON Cyberpunk Commands ---\033[0m");
    Serial.println("  lock XX:XX:XX:XX:XX:XX : Target lock vector line & crosshair");
    Serial.println("  unlock                 : Clear target lock");
    Serial.println("  crt                    : Toggle CRT scanline filter");
    ...
    Serial.println("  hud                    : Toggle Live Terminal HUD");
    Serial.println("  j or json              : Toggle compact JSON stream");
    Serial.println("  db or export           : Dump full active database");
    Serial.println("  l                      : Toggle live text stream");
    Serial.println("  auto                   : Toggle auto-view switching");
...
void drawHeader(uint16_t textColor, const char* title) {
  // 1. Airspace Threat Grade Badge [A] (X: 0..17)
  char grade = 'A';
  int threatScore = calculateThreatScore(grade);
  ...
  // 2. Active Devices Count & Max Distance Telemetry (X: 19..61)
  canvas.setCursor(19, 3);
  canvas.setTextColor(textColor);
  canvas.print(title);

  // 3. Real-Time Clock (X: 64..95)
  ...
  // 4. Smooth Channel Hopping Tape (X: 100..159)
  drawChannelTape();
...
void drawRadar() {
  ...
  // MAX-AREA RADAR SCOPE (Radius 46, fitting Y: 19 to 111, X: 5 to 97)
  const int radarCenterX = 51;
  const int radarCenterY = 65;
  const int radarRadius = 46;
  ...
  // Build unified list of active devices (Wi-Fi or BLE based on currentRadarMode) sorted by RSSI
  struct RadarItem {
    uint8_t mac[6];
    int rssi;
    float dist;
  ...
  if (currentRadarMode == RADAR_WIFI) {
    ...
    for (int i = 0; i < MAX_APS; i++) {
      if (trackedAPs[i].active && totalCount < 60) {
        ...
        if (strlen(trackedAPs[i].ssid) > 0) {
          snprintf(item.label, sizeof(item.label), "%.8s", trackedAPs[i].ssid);
        } else {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
      if (trackedClients[i].active && totalCount < 60) {
        ...
        item.isBT = false;
        if (strlen(trackedClients[i].probedSSID) > 0) {
          snprintf(item.label, sizeof(item.label), "%.8s", trackedClients[i].probedSSID);
        } else {
          const char* vendor = getVendor(item.mac);
          snprintf(item.label, sizeof(item.label), "%.8s", vendor);
        }
        item.color = (item.rssi > -60) ? FIX_GREEN : ((item.rssi > -80) ? FIX_CYAN : FIX_RED);
      }
    }
  } else {
    // 3. Add active Bluetooth BLE Devices
    for (int i = 0; i < MAX_BLE_DEVICES; i++) {
  ...
  // Render Dynamic Header replacing static RADAR text
  float farthestDist = (totalCount > 0) ? items[totalCount - 1].dist : 0.0f;
  char headerBuf[20];
  if (currentRadarMode == RADAR_WIFI) {
    if (totalCount > 0) snprintf(headerBuf, sizeof(headerBuf), "WF:%dD %.0fm", totalCount, farthestDist);
    else snprintf(headerBuf, sizeof(headerBuf), "WF:0D 0m");
    drawHeader(FIX_GREEN, headerBuf);
  } else {
    if (totalCount > 0) snprintf(headerBuf, sizeof(headerBuf), "BT:%dD %.0fm", totalCount, farthestDist);
    else snprintf(headerBuf, sizeof(headerBuf), "BT:0D 0m");
    drawHeader(FIX_MAGENTA, headerBuf);
  }

  // Draw Blips on Max Radar Scope (ALL active devices in range!)
  for (int i = 0; i < totalCount; i++) {
    ...
    // 1. Comet Trail (Fading tail behind blip)
    ...
    // 2. Encryption Pulsing Aura for OPEN/WEP APs
    if (items[i].isAP) {
      bool isUnencrypted = false;
      for (int k = 0; k < MAX_APS; k++) {
        if (trackedAPs[k].active && memcmp(trackedAPs[k].bssid, items[i].mac, 6) == 0) {
          if (strcmp(trackedAPs[k].encryption, "OPEN") == 0 || strcmp(trackedAPs[k].encryption, "WEP") == 0) {
            isUnencrypted = true;
          }
          break;
        }
      ...
      if (isUnencrypted) {
        float pulse = sin(millis() * 0.008f);
        int haloR = 4 + (int)(pulse * 2.5f);
        if (haloR < 3) haloR = 3;
        canvas.drawCircle(bx, by, haloR, FIX_RED);
      }
      canvas.drawCircle(bx, by, 3, items[i].color);
    } else {
      // 3. MAC Randomization Indicator (Hollow Triangle for Randomized MACs)
    ...
    // 4. Target Vector Line & Rotating Crosshair Mode
  ...
  for (int i = 0; i < totalCount && i < 4; i++) {
    if (items[i].isBT) {
      canvas.fillRect(104, yPos + 1, 4, 4, FIX_MAGENTA);
      canvas.setCursor(110, yPos);
      canvas.setTextColor(FIX_MAGENTA);
      canvas.print("BT");
    } else if (items[i].isAP) {
      canvas.drawCircle(105, yPos + 3, 2, items[i].color);
      canvas.setCursor(110, yPos);
      canvas.setTextColor(FIX_YELLOW);
      canvas.print("AP");
    } else {
      canvas.fillRect(104, yPos + 1, 4, 4, items[i].color);
      canvas.setCursor(110, yPos);
      canvas.setTextColor(FIX_CYAN);
      canvas.print("CL");
    }

    canvas.setCursor(130, yPos);
    canvas.setTextColor(FIX_WHITE);
  ...
  // Draw Marquee Ticker & apply CRT Scanlines
  drawMarqueeTicker();
  applyScanlines();
  tft.drawRGBBitmap(0, 0, canvas.getBuffer(), 160, 128);
}
...
void loop() {
  ...
  // 10-Second Auto-Toggle between Wi-Fi Radar and Bluetooth BLE Radar
  if (now - lastRadarModeToggle > 10000) {
    currentRadarMode = (currentRadarMode == RADAR_WIFI) ? RADAR_BLE : RADAR_WIFI;
    lastRadarModeToggle = now;
  }

  // Trigger non-blocking BLE Scan (every 1 second)
  static unsigned long lastBLEScanTrigger = 0;
  if (now - lastBLEScanTrigger > 1000) {
    if (pBLEScan) {
  ...
  for (int i = 0; i < MAX_BLE_DEVICES; i++) {
     if (trackedBLE[i].active && (now - trackedBLE[i].lastSeen > 30000)) {
         trackedBLE[i].active = false;
     }
  }

  // 3. Update the Visual Radar screen (every 250ms for smooth animations)
  static unsigned long lastUIDraw = 0;
  if (now - lastUIDraw > 250) {
    drawRadar(); // Dedicated Visual Radar View Only
    lastUIDraw = now;
  }

  // 4. Refresh Terminal Console HUD (every 1 second)
